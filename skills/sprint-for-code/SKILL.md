...
description: >
  问题驱动的软件工程工作流。仅当任务的主要产出是软件仓库变更、实现 / PR / diff
  代码审查结论，或基于可重复测量的性能 / 可靠性优化结果时使用；包括代码、配置、
  测试、构建脚本及与实现绑定的仓库工程文档。修复 bug 使用本 skill；仅调查原因而不修改时
  使用 sprint-for-analysis。不要用于代码解释或一般技术问答、非代码调研 / 决策 / 内容审阅、
  摘要 / 翻译 / 改写、头脑风暴或无需工程工作流的简单命令。
---
...
- 诊断并修复软件问题属于本 skill；只调查原因、不修改仓库时使用 `sprint-for-analysis`。
- PR、diff 或实现代码审查属于本 skill；需求、报告、论证或事故文档审阅使用 `sprint-for-analysis`。
- 与当前实现绑定的仓库工程文档修改属于本 skill；普通内容创作、摘要、翻译或改写不属于本 skill。
...
适用：实现验收、PR / diff 代码审查、与当前实现绑定的仓库工程文档 review。

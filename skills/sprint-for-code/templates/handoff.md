...
PLACEHOLDERS:
  ...
  {DESC}         用户输入 desc
  {SEQUENCE}     完整真实执行语法；只允许 `stage` / `loop(stageA → stageB) max=N` / `parallel(stageA, stageB, ...)`，顶层 token 用 ` → ` 连接
  {CREATED}      ISO8601 UTC 创建时刻
  {SEQUENCE_ADJUST_REASON_LINE}  sequence 调整原因整行；有调整时写 `  sequence_adjust_reason: 加 verify — 需要补验证`；无调整时整行省略
-->
...
PLACEHOLDERS:
  ...
  {COMPLETED}   parallel 已完成任务列表；YAML 行内数组，如 `[]` 或 `[task-a, task-b]`
-->
